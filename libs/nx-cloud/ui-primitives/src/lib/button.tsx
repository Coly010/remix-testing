...
type AllowedVariants = 'primary' | 'secondary' | 'danger';
...
const variantStyles: Record<AllowedVariants, string> = {
  ...
  secondary:
    'border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 shadow-sm group-hover:bg-slate-50 dark:group-hover:bg-slate-700 group-focus:ring-2 group-focus:ring-blue-500 dark:group-focus:ring-sky-500 group-focus:ring-offset-2',
  danger:
    'border border-transparent bg-red-600 dark:bg-red-500 font-medium text-white shadow-sm group-hover:bg-red-700 dark:group-hover:bg-red-600 group-focus:outline-none group-focus:ring-2 group-focus:ring-red-500 dark:group-focus:ring-red-600 group-focus:ring-offset-2',
};
...
function getLayoutClassName({ className = '' }: { className: string }): string {
  return cx(
    'group relative inline-flex rounded shadow-sm font-medium opacity-100 focus:outline-none disabled:opacity-80 disabled:cursor-not-allowed transition',
    className
  );
}
